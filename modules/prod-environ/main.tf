# create a security group for the production environment
...
# create a security group for the production ALB
...
# create an IAM role for the production environment
resource "aws_iam_role" "prod_role" {
  name = "${var.name}-prod-role"
...
# create an IAM role policy attachment to attach the AmazonSSMManagedInstanceCore policy to the production role
resource "aws_iam_role_policy_attachment" "ssm" {
  role       = aws_iam_role.prod_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

# create an IAM instance profile for the production environment
resource "aws_iam_instance_profile" "prod_profile" {
  name = "${var.name}-prod-profile"
  role = aws_iam_role.prod_role.name
}
...
resource "aws_launch_template" "prod" {
  ...
  iam_instance_profile {
    name = aws_iam_instance_profile.prod_profile.name
  }
...
# create an autoscaling group for the production environment
...
# create a load balancer for the production environment
...
# create a target group for the production environment
...
# create an HTTPS listener for the production environment
...
# create a Route 53 record for the production environment
